...
namespace LM.App.Wpf.ViewModels
{
    ...
    public sealed class LibraryViewModel : ViewModelBase
    {
        ...
        {
            var list = new System.Collections.Generic.List<EntryType>();
    ...
    $"YearFrom={filter.YearFrom}, YearTo={filter.YearTo}, IsInternal={filter.IsInternal}"
                ...
                System.Diagnostics.Debug.WriteLine($"[LibraryViewModel] SearchAsync → {rows.Count} rows");
                ...
                if (rows.Count == 0)
                    System.Diagnostics.Debug.WriteLine("[LibraryViewModel] No entries matched filter");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[LibraryViewModel] SearchAsync FAILED: {ex}");
